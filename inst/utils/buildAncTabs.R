...
pfiles = c(
 mff = system.file("extdata", "go323", "go_mf_parents.parquet", package="GO.db3"),
 ccf = system.file("extdata", "go323", "go_cc_parents.parquet", package="GO.db3"),
 bpf = system.file("extdata", "go323", "go_bp_parents.parquet", package="GO.db3")
)

make_anc_table = function(fi) {
 cur = read_parquet(fi)
 curh = filter(cur, relationship_type %in% c("is_a", "part_of", "regulates")) 
 cur = curh |> as.data.frame() |> mutate(id=go_id, parent=parent_id) 
 find_all_ancestors(cur)
}

tabs = lapply(pfiles, make_anc_table)
saveRDS(tabs$mff, "mfancestorstab.rds")
saveRDS(tabs$ccf, "ccancestorstab.rds")
saveRDS(tabs$bpf, "bpancestorstab.rds")
